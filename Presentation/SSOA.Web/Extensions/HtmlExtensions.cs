...
namespace SSOA.Web.Extensions
{
    public static class HtmlExtensions
    {
        ...
        public static string GetTopicSeName<T>(this HtmlHelper<T> html, string systemName)
        {
            ...
            var cacheManager = EngineContext.Current.ContainerManager.Resolve<ICacheManager>("SSOA_cache_static");
            var cacheKey = string.Format(ModelCacheEventConsumer.TOPIC_SENAME_BY_SYSTEMNAME, systemName, workContext.WorkingLanguage.Id, storeContext.CurrentTenant.Id);
            var cachedSeName = cacheManager.Get(cacheKey, () =>
            {
                var topicService = EngineContext.Current.Resolve<ITopicService>();
                var topic = topicService.GetTopicBySystemName(systemName, storeContext.CurrentTenant.Id);
                if (topic == null)
                    return "";
