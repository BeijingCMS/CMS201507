    ...
    public partial class BoardsController : BasePublicController
    {
        ...
        public ActionResult ActiveDiscussionsRss(int forumId = 0)
        {
            ...
            var feed = new SyndicationFeed(
                                    string.Format(feedTitle, _storeContext.CurrentTenant.GetLocalized(x => x.Name)),
                                    feedDescription,
                                    new Uri(url),
        ...
        public ActionResult ForumRss(int id)
        {
            ...
            if (forum != null)
            {
                ...
                var feed = new SyndicationFeed(
                                        string.Format(feedTitle, _storeContext.CurrentTenant.GetLocalized(x => x.Name), forum.Name),
                                        feedDescription,
                                        new Uri(url),
            ...
            string within, string limitDays, int page = 1)
        {
            ...
            try
            {
                if (!String.IsNullOrWhiteSpace(searchterms))
                {
                    ...
                    {
                        throw new SSOAException(string.Format(_localizationService.GetResource("Forum.SearchTermMinimumLengthIsNCharacters"),
