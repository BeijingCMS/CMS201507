CREATE FUNCTION [dbo].[SSOA_splitstring_to_table]
(
    @string NVARCHAR(MAX),
    @delimiter CHAR(1)
)
...
CREATE FUNCTION [dbo].[SSOA_getnotnullnotempty]
(
...
CREATE FUNCTION [dbo].[SSOA_getprimarykey_indexname]
(
...
BEGIN
	...
	SELECT CAST(data as int) FROM [SSOA_splitstring_to_table](@CategoryIds, ',')	
	...
	SELECT CAST(data as int) FROM [SSOA_splitstring_to_table](@FilteredSpecs, ',')
	...
	SELECT CAST(data as int) FROM [SSOA_splitstring_to_table](@AllowedCustomerRoleIds, ',')
...
BEGIN
	--create catalog
	EXEC('
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''SSOACommerceFullTextCatalog'')
		CREATE FULLTEXT CATALOG [SSOACommerceFullTextCatalog] AS DEFAULT')
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[Product]''))
		...
		KEY INDEX [' + dbo.[SSOA_getprimarykey_indexname] ('Product') +  '] ON [SSOACommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[LocalizedProperty]''))
		...
		KEY INDEX [' + dbo.[SSOA_getprimarykey_indexname] ('LocalizedProperty') +  '] ON [SSOACommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductTag]''))
		...
		KEY INDEX [' + dbo.[SSOA_getprimarykey_indexname] ('ProductTag') +  '] ON [SSOACommerceFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
END
GO
...
BEGIN
	...
	--drop catalog
	EXEC('
	IF EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''SSOACommerceFullTextCatalog'')
		DROP FULLTEXT CATALOG [SSOACommerceFullTextCatalog]
	')
END
GO
