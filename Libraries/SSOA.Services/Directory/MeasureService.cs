...
namespace SSOA.Services.Directory
{
    /// <summary>
    ...
    public partial class MeasureService : IMeasureService
    {
        ...
        /// Key for caching
        /// </summary>
        private const string MEASUREDIMENSIONS_ALL_KEY = "SSOA.measuredimension.all";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        /// {0} : dimension ID
        /// </remarks>
        private const string MEASUREDIMENSIONS_BY_ID_KEY = "SSOA.measuredimension.id-{0}";
        /// <summary>
        /// Key for caching
        /// </summary>
        private const string MEASUREWEIGHTS_ALL_KEY = "SSOA.measureweight.all";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        /// {0} : weight ID
        /// </remarks>
        private const string MEASUREWEIGHTS_BY_ID_KEY = "SSOA.measureweight.id-{0}";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string MEASUREDIMENSIONS_PATTERN_KEY = "SSOA.measuredimension.";
        /// <summary>
        /// Key pattern to clear cache
        /// </summary>
        private const string MEASUREWEIGHTS_PATTERN_KEY = "SSOA.measureweight.";
            ...
            MeasureDimension sourceMeasureDimension)
        {
            ...
            if (result != decimal.Zero && sourceMeasureDimension.Id != baseDimensionIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new SSOAException(string.Format("Exchange ratio not set for dimension [{0}]", sourceMeasureDimension.Name));
            ...
            MeasureDimension targetMeasureDimension)
        {
            ...
            if (result != decimal.Zero && targetMeasureDimension.Id != baseDimensionIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new SSOAException(string.Format("Exchange ratio not set for dimension [{0}]", targetMeasureDimension.Name));
                result = result * exchangeRatio;
            }
            return result;
        }
        ...
        public virtual decimal ConvertToPrimaryMeasureWeight(decimal value, MeasureWeight sourceMeasureWeight)
        {
            ...
            if (result != decimal.Zero && sourceMeasureWeight.Id != baseWeightIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new SSOAException(string.Format("Exchange ratio not set for weight [{0}]", sourceMeasureWeight.Name));
                result = result / exchangeRatio;
            }
            return result;
        }
            ...
            MeasureWeight targetMeasureWeight)
        {
            ...
            if (result != decimal.Zero && targetMeasureWeight.Id != baseWeightIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new SSOAException(string.Format("Exchange ratio not set for weight [{0}]", targetMeasureWeight.Name));
                result = result * exchangeRatio;
            }
            return result;
        }
