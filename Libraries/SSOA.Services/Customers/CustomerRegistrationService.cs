...
namespace SSOA.Services.Customers
{
    /// <summary>
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual CustomerRegistrationResult RegisterCustomer(CustomerRegistrationRequest request)
        {
            ...
            if (registeredRole == null)
                throw new SSOAException("'Registered' role could not be loaded");
        ...
        public virtual void SetEmail(Customer customer, string newEmail)
        {
            ...
            if (newEmail == null)
                throw new SSOAException("Email cannot be null");

            newEmail = newEmail.Trim();
            string oldEmail = customer.Email;

            if (!CommonHelper.IsValidEmail(newEmail))
                throw new SSOAException(_localizationService.GetResource("Account.EmailUsernameErrors.NewEmailIsNotValid"));

            if (newEmail.Length > 100)
                throw new SSOAException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailTooLong"));

            var customer2 = _customerService.GetCustomerByEmail(newEmail);
            if (customer2 != null && customer.Id != customer2.Id)
                throw new SSOAException(_localizationService.GetResource("Account.EmailUsernameErrors.EmailAlreadyExists"));
        ...
        public virtual void SetUsername(Customer customer, string newUsername)
        {
            ...
            if (!_customerSettings.UsernamesEnabled)
                throw new SSOAException("Usernames are disabled");

            if (!_customerSettings.AllowUsersToChangeUsernames)
                throw new SSOAException("Changing usernames is not allowed");

            newUsername = newUsername.Trim();

            if (newUsername.Length > 100)
                throw new SSOAException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameTooLong"));

            var user2 = _customerService.GetCustomerByUsername(newUsername);
            if (user2 != null && customer.Id != user2.Id)
                throw new SSOAException(_localizationService.GetResource("Account.EmailUsernameErrors.UsernameAlreadyExists"));
